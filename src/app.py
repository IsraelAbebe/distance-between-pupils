...
def predict(path):
    ...
    for (i, rect) in enumerate(rects):
        ...
        face_pixel_left = [1,33]
        face_left_dist = 0
        for i in range(len(face_pixel_left)-1):
            face_left_dist += np.linalg.norm(shape[i+1]-shape[i])

        # length of face end to end
        face_pixel_right = [33,17]
        face_right_dist = 0
        for i in range(len(face_pixel_right)-1):
            face_right_dist += np.linalg.norm(shape[i+1]-shape[i])
        ...
        REAL_left_lash_dist = (left_lash_dist *3.375)/width
        REAL_face_dist = ((face_left_dist *3.375)/width)+((face_right_dist *3.375)/width)
        

        return_dict = {}

        return_dict['pupil_distance']=real_mesurement
        return_dict['left_eye_lash']=REAL_left_lash_dist
        return_dict['right_eye_lash']=REAL_right_lash_dist
        return_dict['jaw']=REAL_jaw_dist
        return_dict['nose']=REAL_nose_dist
        return_dict['upper_lip']=REAL_upper_lip_dist
        return_dict['ear_to_ear']=REAL_face_dist
...
# print(predict("images/x.jpg"))


def face_measure(input: FaceMeasureInput,) -> Output:
    result = []
    ...
    return Output(message=dict_mean(result))



# check if its left or right facing 
# calculate half face distance for them
# add those
